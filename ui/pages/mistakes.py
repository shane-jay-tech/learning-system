...
def _render_due_reviews(dao, reviews):
    ...
    for r in reviews:
        ...
        topic_obj, problem_obj = find_problem(r["lang"], r["problem_id"])
...
def _render_weak_topics(dao, mistakes):
    section_title("弱项专练")
    st.caption("按 topic 分组，集中攻克薄弱环节。")

    topic_stats = {}
    for m in mistakes:
        topic_obj, _ = find_problem(m["lang"], m["problem_id"])
        if topic_obj:
        ...
        with st.container(border=True):
            ...
            with col_r:
                if st.button("练习 →", key=f"weak_{t['lang']}_{t['slug']}",
                    ...
                    st.session_state.lang = t["lang"]
                    st.session_state.topic = t["slug"]
                    st.rerun()
