> 2026-09-17 数字复核：题量 400→399、用例 311→383（376 passed/7 skipped），经 generate_metrics 实测与 pytest 实测对平。
> 2026-09-18 数字复核：用例 383→391（384 passed/7 skipped，零 failed）；2026-09-19 复测 402（395 passed/7 skipped，l918-04/05 补测后）。
...
| 代码沙箱 | 4 种 |
| 代码行数 | ~10,900 行 |
| 测试用例 | 383 个（376 passed, 7 skipped）〔9/18 复核：391=384 passed/7 skipped；9/19：402=395/7〕 |
| 数据库表 | 7 张 |
...
| 优先级 | 方向 |
|--------|------|
| P1 | AI 调用进程复用（常驻 llm 服务，省每次 100-300ms 子进程启动；需改仓库外 D:/code/scripts/llm_call.py） |
| P3 | （无） |
